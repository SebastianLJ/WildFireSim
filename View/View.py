...
def animate(i,im,model,prediction_model,window):
    ...
    while True:
            ...
            if event == '-START-':
                if generated_flag:
                    ...
                    prediction_model = CombustionModel(128, 128, selected_seed, True, 4)
                    log = Log()
                    ...
                    ax2.set_axis_off()
                    ax2.imshow(test_terrain.terrainMap, cmap=cmap_spread, norm=norm_spread)
                    ...
                    log.add(model.time, model.FireModel.fireMap, prediction_model.FireModel.fireMap)
                    anim = animation.FuncAnimation(fig, animate, interval=interval, frames=300, fargs=(im,model,prediction_model,window))
                    # anim.save("forest_fire.mp4")
